...
export default function FreelancerHome() {
    ...
    const fetchMatchedJobs = async () => {
      ...
      try {
        const response = await apiClient.get(
          `/freelancer/matched-jobs/${user._id}`,
        );
  ...
  // Debounce keyword changes — require at least 3 chars for keyword-only searches
  useEffect(() => {
    const keywordTooShort = keyword.trim().length > 0 && keyword.trim().length < 3;
    if (keywordTooShort && !category && !experienceLevel) return;
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-white/80 mb-4">
                      Set up your freelancer profile to start seeing job
            ...
            {!isSearchMode &&
              ...
              matchedJobs.length === 0 && (
                <div className="p-10 rounded-2xl bg-[var(--color-card)] border border-[var(--color-border)] text-center">
                  ...
                  <p className="mt-2 text-[var(--color-text-secondary)] max-w-md mx-auto">
                    We're working on finding the best jobs for your skills.
                    Check back soon!
                  </p>
                </div>
              )}
